    ...
    <>
      ...
      >
        ...
        <div className="ml-[1rem]">
          ...
          <p className="text-[60%] my-[1%]">{content.description}</p>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50">
          <div className="bg-white p-8 pb-4  w-[40%] h-[50%] flex flex-col items-start justify-around rounded-xl">
            ...
            <h2 className="text-[28px] font-bold">{content.title}</h2>
            <p>{content.budget}</p>
            <p className="mt-2">{content.description}</p>
            <p>{content.date}</p>
            <div>
              <button className="mt-4 bg-blue-500 text-white px-4  m-2 py-2 rounded" onClick={closeModal}>Accept Request</button>
